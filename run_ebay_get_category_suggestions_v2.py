...
def main():
    ...
    (EXPORTS_DIR / "ebay_category_suggestions_v2.json").write_text(json.dumps(sug_data, ensure_ascii=False, indent=2), encoding="utf-8")
    (EXPORTS_DIR / "ebay_category_suggestions_audit_v2.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    print("GET_CATEGORY_SUGGESTIONS_V2_OK")
    print("categoryTreeId =", tree_id)
    print("suggestion_count =", len(suggestions))
    for i, item in enumerate(suggestions, 1):
        cat = item.get("category", {})
        print(str(i) + ". id =", cat.get("categoryId"), "| name =", cat.get("categoryName"))

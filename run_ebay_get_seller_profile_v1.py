...
def main() -> None:
    ...
    try:
        ...
        (EXPORTS_DIR / "ebay_get_seller_profile_audit_v1.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
        print("GET_SELLER_PROFILE_FAILED")
        print("http_status =", e.code)
        print(body)
        raise SystemExit(1)

    data = json.loads(body)
    (EXPORTS_DIR / "ebay_seller_profile_v1.json").write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    audit = {"status": "OK", "http_status": status_code, "top_keys": list(data.keys())[:10]}
    (EXPORTS_DIR / "ebay_get_seller_profile_audit_v1.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")

    print("GET_SELLER_PROFILE_OK")

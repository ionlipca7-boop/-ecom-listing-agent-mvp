...
INPUT_FILE = BASE_DIR / "storage" / "control_actions.json"
OUTPUT_FILE = EXPORTS_DIR / "control_action_v3.json"
...
def main():
    ...
    for item in actions:
        ...
        if not refresh_ok:
            results.append({"action": action, "sku": sku, "offerId": offer_id, "quantity": quantity, "status": "SKIPPED", "http_status": None, "response_json": {"reason": "REFRESH_FAILED"}})
            overall_ok = False
            continue
        if action == "update_quantity" and sku and offer_id and quantity > 0:
            ...
            results.append({"action": action, "sku": sku, "offerId": offer_id, "quantity": quantity, "status": item_status, "http_status": response.status_code, "request_payload": payload, "response_json": data})
        else:
            results.append({"action": action, "sku": sku, "offerId": offer_id, "quantity": quantity, "status": "FAILED", "http_status": None, "response_json": {"reason": "INVALID_ACTION_OR_DATA"}})
            overall_ok = False

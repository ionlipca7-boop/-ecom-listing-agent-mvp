...
def main():
    ...
    out_path = EXPORT_DIR / "adapter_001_live_offer_v3.json"
    audit_path = EXPORT_DIR / "adapter_001_live_offer_audit_v3.json"
    if response.status_code == 200:
        out_path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    price = data.get("pricingSummary", {}).get("price", {})
    audit = {"status": "OK" if response.status_code == 200 else "ERROR", "decision": "adapter_001_live_offer_v3_fetched" if response.status_code == 200 else "adapter_001_live_offer_v3_fetch_failed", "http_status": response.status_code, "offerId": offer_id, "sku": data.get("sku"), "availableQuantity": data.get("availableQuantity"), "price_value": price.get("value"), "price_currency": price.get("currency")}
    audit_path.write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    print("ADAPTER_001_LIVE_OFFER_V3_FETCH_DONE")

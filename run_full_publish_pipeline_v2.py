...
PIPELINE_AUDIT_FILE = EXPORTS_DIR / "full_publish_pipeline_v2_audit.json"
...
def normalize_aspects(raw_aspects, product):
    result = {}
    if isinstance(raw_aspects, dict):
        for k, v in raw_aspects.items():
            ...
            if key and vals:
                result[key] = vals
    if "Produktart" not in result:
        result["Produktart"] = [str(product.get("produktart") or "Adapter").strip()]
    return result
...
def main():
    ...
    aspects = normalize_aspects(product.get("aspects", {}), product)
    condition_value = normalize_condition(product.get("condition"))
    inventory_payload = {

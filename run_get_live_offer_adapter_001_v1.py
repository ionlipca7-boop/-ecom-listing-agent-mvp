...
def main():
    ...
    out_path = EXPORT_DIR / "adapter_001_live_offer_v1.json"
    audit_path = EXPORT_DIR / "adapter_001_live_offer_audit_v1.json"
    if response.status_code == 200:
        out_path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    audit = {
        "status": "OK" if response.status_code == 200 else "ERROR",
        "decision": "adapter_001_live_offer_fetched" if response.status_code == 200 else "adapter_001_live_offer_fetch_failed",
        ...
        "has_price": bool(data.get("pricingSummary", {}).get("price")),
        "has_policies": bool(data.get("listingPolicies")),
        "categoryId": data.get("categoryId")
    }
    audit_path.write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    print("ADAPTER_001_LIVE_OFFER_V1_FETCH_DONE")

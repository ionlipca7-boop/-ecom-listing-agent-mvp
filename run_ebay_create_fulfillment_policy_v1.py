...
API_URL = "https://api.ebay.com/sell/account/v1/fulfillment_policy"

def main() -> None:
    token = (SECRETS_DIR / "ebay_access_token.txt").read_text(encoding="utf-8").strip()

    payload_obj = {
        "name": "ECOM_DE_FULFILLMENT_STD_V1",
        "description": "Standard DE fulfillment policy created by ECOM Listing Agent MVP",
    ...
    try:
        ...
        (EXPORTS_DIR / "ebay_create_fulfillment_policy_audit_v1.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
        print("CREATE_FULFILLMENT_POLICY_FAILED")
        print("http_status =", e.code)
        print(body)
        raise SystemExit(1)

    data = json.loads(body) if body.strip() else {}
    (EXPORTS_DIR / "ebay_create_fulfillment_policy_v1.json").write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    audit = {"status": "OK", "http_status": status_code, "location": location, "top_keys": list(data.keys())[:10]}
    (EXPORTS_DIR / "ebay_create_fulfillment_policy_audit_v1.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    print("CREATE_FULFILLMENT_POLICY_OK")

...
def main():
    ...
    in_file = root / "storage" / "state_control" / "current_listing_read_plan_v1.json"
    out_dir = root / "storage" / "state_control"
    out_dir.mkdir(parents=True, exist_ok=True)
    data = json.loads(in_file.read_text(encoding="utf-8"))
    audit = {
        "status": "OK",
        "layer": "CURRENT_LISTING_READ_PLAN_AUDIT_V1",
        "project": "ECOM_LISTING_AGENT_MVP",
        "input_layer": data["layer"],
        "input_status": data["status"],
        "scope_target": data["scope_target"],
        "read_plan_defined": data["read_plan_defined"],
        "read_mode": data["read_mode"],
        "read_sequence_count": len(data["read_sequence"]),
        ...
        "audit_result": "current_listing_read_plan_verified",
        "next_allowed_action": "build_current_listing_read_contract_v1"
    }
    out_file = out_dir / "current_listing_read_plan_audit_v1.json"
    out_file.write_text(json.dumps(audit, indent=2), encoding="utf-8")
    print("CURRENT_LISTING_READ_PLAN_AUDIT_V1_CREATED")
    ...
    print("read_plan_defined =", audit["read_plan_defined"])
    print("read_mode =", audit["read_mode"])
    print("read_sequence_count =", audit["read_sequence_count"])

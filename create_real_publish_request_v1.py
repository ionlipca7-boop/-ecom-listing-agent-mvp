...
OUTPUT_FILE = Path("real_publish_request_v1.json")
...
def main() -> int:
    ...
    if not run_ok or not isinstance(run_payload, dict):
        ...
        run_package = run_payload.get("last_package_detected")
        if isinstance(run_package, str) and run_package.strip():
            last_package = run_package.strip()
    ...
    print(f"request_status: {output_payload['request_status']}")
    print(f"output_file: {OUTPUT_FILE.as_posix()}")

    return 0 if output_payload["request_status"] == "CREATED" else 1
